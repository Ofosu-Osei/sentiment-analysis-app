...
def train_nn_model(features, labels, model_path=None):
    ...
    # # Convert one-hot encoded labels back to label format for classification report
    # y_test_label = np.argmax(y_test, axis=1)
    # y_pred_label = np.argmax(y_pred, axis=1)

    # Convert predictions and true labels from one-hot encoded to label format
    # y_pred_label = np.argmax(y_pred, axis=1)
    # y_test_label = np.argmax(y_test, axis=1)

    # print(
    #     "Neural Network Classification Report:\n",
    #     classification_report(y_test_label, y_pred_label, zero_division=0),
    # )
